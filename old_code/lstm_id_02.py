...
from keras.layers import LSTM, Dense, TimeDistributed, Input, concatenate, Embedding, Lambda
...
def model_002(n_step):
    ...
    # Teacher forcing concatenated to the LSTM output
...
def model_003(n_step):
    ...
    # Teacher forcing concatenated to the input

    hidden_size = 256

    X = Input(shape=(n_step, 1), name='X')
    Y_pre = Input(shape=(n_step, 2), name='Y_pre')
    Z = concatenate([Y_pre, X])
...
def model_004(n_step):
    ...
    X = Input(shape=(n_step, 1), name='X')
    X_squ = Lambda(lambda y: squeeze(y, -1))(X)
    # X_squ = squeeze(X, -1)
    X_emb = Embedding(2000, 64, input_length=n_step)(X_squ)
    Y_pre = Input(shape=(n_step, 2), name='Y_pre')
