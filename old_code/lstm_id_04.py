...
from keras.layers import LSTM, Dense, TimeDistributed, Input, concatenate, Embedding, Lambda, Dropout
...
data_file = os.path.join('..', 'labeled', 'AE_data.mat')
# data_file = os.path.join('./Preprocess_Data', 'AE_data_casc_512_128.mat')
# data_file = "AE_data_casc_512_128.mat"
...
def load_casc_data(data_file):
    mat_data = sio.loadmat(data_file)

    train_x = mat_data['d_train']
    train_y = mat_data['l_train']
    test_x = mat_data['d_test']
    test_y = mat_data['l_test']
    ...
    return train_x, train_y, test_x, test_y, n_step
...
def load_data(data_file, split_ratio, batch_size, teach_force=False):
    ...
    if teach_force:
        ...
        train_y_pre = to_categorical(train_y_pre, num_classes=2)
        valid_y_pre = to_categorical(valid_y_pre, num_classes=2)
        test_y_pre = to_categorical(test_y_pre, num_classes=2)
...
def model_002(n_step):
    ...
    X = Input(shape=(n_step, 1), name='X')
    Y_pre = Input(shape=(n_step, 2), name='Y_pre')
    LSTM_out1 = LSTM(units=hidden_size, return_sequences=True, input_shape=(n_step, 1))(X)
    LSTM_out2 = LSTM(units=hidden_size, return_sequences=True)(LSTM_out1)
    Z = concatenate([Y_pre, LSTM_out2])
    Y = TimeDistributed(Dense(2, activation='softmax'), name='Y')(Z)
...
def model_003(n_step):
    ...
    X = Input(shape=(n_step, 1), name='X')
    X_emb = Dense(emb_size, activation=activation)(X)
    LSTM_out1 = LSTM(units=hidden_size, return_sequences=True, input_shape=(n_step, emb_size))(X_emb)
    LSTM_out2 = LSTM(units=hidden_size, return_sequences=True)(LSTM_out1)
    Y = TimeDistributed(Dense(2, activation='softmax'), name='Y')(LSTM_out2)
    model = Model(inputs=X, outputs=Y)
    model.summary()
...
def model_004(n_step):
    # Dense first layer & teacher force
    ...
    X_emb = Dense(emb_size, activation=activation)(X)
    Y_pre = Input(shape=(n_step, 2), name='Y_pre')
    Z = concatenate([Y_pre, X_emb])
    LSTM_out1 = LSTM(units=hidden_size, return_sequences=True, input_shape=(n_step, 1))(Z)
    LSTM_out2 = LSTM(units=hidden_size, return_sequences=True)(LSTM_out1)
    Y = TimeDistributed(Dense(2, activation='softmax'), name='Y')(LSTM_out2)
    model = Model(inputs=[X, Y_pre], outputs=Y)
    model.summary()
...
def model_005(n_step):
    # Basic 2-layer LSTM with dropout (dropout model_001)

    hidden_size = 256

    X = Input(shape=(n_step, 1), name='X')
    LSTM_out1 = LSTM(units=hidden_size, dropout = 0.5, recurrent_dropout = 0.5, return_sequences=True, input_shape=(n_step, 1))(X)
...
def model_006(n_step):

    # Dense layer as the first layer with dropout
    ...
    X = Input(shape=(n_step, 1), name='X')
    X_emb = Dense(emb_size, activation=activation)(X)
    LSTM_out1 = LSTM(units=hidden_size, dropout = 0.5, recurrent_dropout = 0.5, return_sequences=True, input_shape=(n_step, emb_size))(X_emb)
    LSTM_out2 = LSTM(units=hidden_size, dropout = 0.5, recurrent_dropout = 0.5, return_sequences=True)(LSTM_out1)
    Y = TimeDistributed(Dense(2, activation='softmax'), name='Y')(LSTM_out2)
    # Y = Dropout(0.5)(LSTM_out2)
    model = Model(inputs=X, outputs=Y)
    model.summary()
...
if __name__ == '__main__':
    ...
    args = arg_parser()
    # train_x, train_y, test_x, test_y, n_step = load_casc_data(args.data_file)
    # valid_x = test_x
    # valid_y = test_y
    ...
    if args.teach_force:
        ...
        train_x, train_y, valid_x, valid_y, test_x, test_y, n_step = load_data(args.data_file, args.split_ratio, args.batch_size)
    ...
    if args.train_flag:
        ...
        if args.teach_force:
            model.fit({'X': train_x, 'Y_pre': train_y_pre}, {'Y': train_y}, batch_size=args.batch_size,
                      epochs=args.n_epochs, verbose=1, callbacks=[tb, cp],
            ...
            model.fit(train_x, train_y, batch_size=args.batch_size, epochs=args.n_epochs, verbose=1,
                      validation_data=(valid_x, valid_y), callbacks=[tb, cp])
        ...
        for i in range(n_iter):
            ...
            pre[i], rec[i], acc[i], f1[i] = perf_measure(test_y.reshape(-1), pred.reshape(-1))
            test_y_pre = to_categorical(np.concatenate(([0], pred.reshape(-1)[:-1])).reshape(-1, n_step), num_classes=2)
            test_y_pre[0, 0, :] = y_pre_1
            print(i, acc[i])
