    ...
    with tf.variable_scope('depth_net') as sc:                          # design the nn architecture for the depth network
        ...
        with slim.arg_scope([slim.conv2d, slim.conv2d_transpose],       #define a conv2d operator with fixed params shown below
            ...
            with tf.variable_scope('mask'):
                ...
                disp4  = SCALING * slim.conv2d(icnv4, num_plane + 1,   [3, 3], stride=1,
                        activation_fn=None, normalizer_fn=None, scope='disp4') + BIAS                 # 4*24*40*(1+n)
                disp4_up = tf.cast(tf.image.resize_bilinear(disp4, [np.int(H/4), np.int(W/4)]), tf.float64)
                ...
                disp3  = SCALING * slim.conv2d(icnv3, num_plane + 1,   [3, 3], stride=1,               #4*48*80*(1+n)
                    activation_fn=None, normalizer_fn=None, scope='disp3') + BIAS
                disp3_up = tf.cast(tf.image.resize_bilinear(disp3, [np.int(H/2), np.int(W/2)]), tf.float64)
                ...
                disp2  = SCALING * slim.conv2d(icnv2, num_plane + 1,   [3, 3], stride=1,                 #4*96*160*(n+1)
                    activation_fn=None, normalizer_fn=None, scope='disp2') + BIAS
                disp2_up = tf.cast(tf.image.resize_bilinear(disp2, [H, W]), tf.float64)
                ...
                disp1  = SCALING * slim.conv2d(icnv1, num_plane + 1,   [3, 3], stride=1,
                    activation_fn=None, normalizer_fn=None, scope='disp1') + BIAS

            end_points = utils.convert_collection_to_dict(end_points_collection)
            return param_final, [disp1, disp2, disp3, disp4], end_points

tgt_image = np.random.rand(4, 192, 320, 1)
num_plane = 0
plane_pred_net(tgt_image, num_plane, is_training=True)
print('asd')